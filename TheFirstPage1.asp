...
If (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    Set MM_editCmd = Server.CreateObject ("ADODB.Command")
    MM_editCmd.ActiveConnection = MM_chris_STRING
    MM_editCmd.CommandText = "DELETE FROM dbo.Bow1 WHERE Bow1_num = ?"
...
Recordset1_cmd.CommandText = "SELECT * FROM dbo.Bow1" 
...
<div class="container-fluid"> 
  <div class="collapse navbar-collapse" id="defaultNavbar1">
      <ul class="nav navbar-nav">
        ...
        <li><a href="Footprints.asp">借用记录</a></li> 
...
<table class="table table-striped" style="margin-left:50px;margin-right:150px; background-color:#F0EBEB; width:1260px">
	<caption style="font-size:24px; background-color:#F0EBEB; text-align:center"><strong>请选择要借用的教室：</strong>
...
<button class=" btn-sm"	><A HREF="<%=MM_moveNext%>">后一个</A></button>
  ...
  <thead>
		...
		<tbody>
          <tr>
            <th><input name="date" type="text" value="<%=(Recordset1.Fields.Item("Bow1_data").Value)%>" readonly></th>
            <th><input name="time" type="text" value="<%=(Recordset1.Fields.Item("Bow1_time").Value)%>" readonly></th>
            <th><input name="classno" type="text" value="<%=(Recordset1.Fields.Item("Bow1_classno").Value)%>" readonly></th>
            <th><input name="reason" type="text" value="<%=(Recordset1.Fields.Item("Bow1_reason").Value)%>" readonly></th>
...
<input type="hidden" name="MM_recordId" value="<%= Recordset1.Fields.Item("Bow1_num").Value %>">
</form>
